...
$logArray = @(
    ...
    '/Windows/ServiceProfiles/LocalService/AppData/Local/Microsoft/WSLicense/tokens.dat'
...
forEach ( $partitionGroup in $partitionlist | group DiskNumber ) {
    ...
    # If both was found grab bcd store and root partition log files
    if ( $isBcdPath -and $isOsPath ) {

        # Create or get CaseLogs folder on desktop
        if (Test-Path "$($folderPath)$($logFolderName)") {
            $folder = Get-Item -Path "$($folderPath)$($logFolderName)"
            Write-Host "Grabbing folder $($folder)"
        }
        else {
            $folder = New-Item -Path $folderPath -Name $logFolderName -ItemType "directory"
            Write-Host "Creating folder $($folder)"
        }

        # Create folder named after the time current
        $subFolder = New-Item -Path $folder.ToString() -Name $scriptStartTime -ItemType "directory"
...
# Copy-Item -Path "C:\fcshell\*" -Destination $folder -Recurse
# $folder.PSPath
# $folder.ToString()
